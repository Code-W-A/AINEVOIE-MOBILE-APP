...
const COUNTY_DEFINITIONS: readonly CountyDefinition[] = Object.freeze([
...
function withDiacriticsFallback(value: string) {
  ...
  if (normalized === 'Romania') return 'România';
  if (normalized === 'Bucuresti') return 'București';
  if (normalized === 'Timis') return 'Timiș';
  if (normalized === 'Maramures') return 'Maramureș';
  if (normalized === 'Mehedinti') return 'Mehedinți';
  if (normalized === 'Mures') return 'Mureș';
  if (normalized === 'Neamt') return 'Neamț';
  if (normalized === 'Salaj') return 'Sălaj';
  if (normalized === 'Valcea') return 'Vâlcea';
  if (normalized === 'Iasi') return 'Iași';
  if (normalized === 'Bistrita-Nasaud') return 'Bistrița-Năsăud';
  if (normalized === 'Caras-Severin') return 'Caraș-Severin';
  if (normalized === 'Calarasi') return 'Călărași';
  if (normalized === 'Dambovita') return 'Dâmbovița';
  if (normalized === 'Timisoara') return 'Timișoara';
  if (normalized === 'Pitesti') return 'Pitești';
  if (normalized === 'Campulung') return 'Câmpulung';
  if (normalized === 'Curtea de Arges') return 'Curtea de Argeș';
  if (normalized === 'Botosani') return 'Botoșani';
  if (normalized === 'Brasov') return 'Brașov';
  if (normalized === 'Buzau') return 'Buzău';
  if (normalized === 'Resita') return 'Reșița';
  if (normalized === 'Constanta') return 'Constanța';
  if (normalized === 'Sfantu Gheorghe') return 'Sfântu Gheorghe';
  if (normalized === 'Targoviste') return 'Târgoviște';
  if (normalized === 'Galati') return 'Galați';
  if (normalized === 'Targu Jiu') return 'Târgu Jiu';
  if (normalized === 'Targu Mures') return 'Târgu Mureș';
  if (normalized === 'Piatra Neamt') return 'Piatra Neamț';
  if (normalized === 'Valenii de Munte') return 'Vălenii de Munte';
  if (normalized === 'Zalau') return 'Zalău';
  if (normalized === 'Campulung Moldovenesc') return 'Câmpulung Moldovenesc';
  if (normalized === 'Ramnicu Valcea') return 'Râmnicu Vâlcea';
  if (normalized === 'Focsani') return 'Focșani';
  if (normalized === 'Baicoi') return 'Băicoi';
  if (normalized === 'Sannicolau Mare') return 'Sânnicolau Mare';
  if (normalized === 'Targu Neamt') return 'Târgu Neamț';
  if (normalized === 'Targu Frumos') return 'Târgu Frumos';
  if (normalized === 'Targu Bujor') return 'Târgu Bujor';
  if (normalized === 'Simleu Silvaniei') return 'Șimleu Silvaniei';
  if (normalized === 'Sangeorz-Bai') return 'Sângeorz-Băi';
  if (normalized === 'Negresti-Oas') return 'Negrești-Oaș';
  if (normalized === 'Moldova Noua') return 'Moldova Nouă';
  if (normalized === 'Dragasani') return 'Drăgășani';
  if (normalized === 'Calimanesti') return 'Călimănești';
  if (normalized === 'Babeni') return 'Băbeni';
...
function buildCityRecord(cityName: string): CityRecord {
...
function buildCountyRecord(county: CountyDefinition): CountyRecord {
...
const ROMANIA_COUNTIES: readonly CountyRecord[] = Object.freeze(
  COUNTY_DEFINITIONS.map((county) => Object.freeze(buildCountyRecord(county))),
);

function getCountyCandidates(county: CountyRecord) {
  return [
    county.countyCode,
    county.countyName,
    ...(Array.isArray(county.aliases) ? county.aliases : []),
...
function getCityCandidates(city: CityRecord) {
  return [
    city.cityCode,
    city.cityName,
...
function isBucharestCounty(county: CountyRecord | null | undefined) {
  return county?.countyCode === BUCHAREST_COUNTY_CODE;
}

function matchesLegacyBucharestCity(value: unknown) {
...
function matchByCandidates(candidates: string[], value: unknown) {
...
function matchIncludesCandidates(candidates: string[], value: unknown) {
  const normalizedValue = normalizeToken(value);
...
export function getRomaniaCountry() {
  return { ...ROMANIA_COUNTRY };
}

export function getRomaniaCounties() {
...
export function getRomaniaCitiesByCounty(countyCode: unknown) {
...
export function findRomaniaCounty(value: unknown) {
  const normalizedValue = normalizeToken(value);
...
export function findRomaniaCity(countyCode: unknown, value: unknown) {
  const county = findRomaniaCounty(countyCode);
  const normalizedValue = normalizeToken(value);
...
export function resolveRomaniaCoverageHierarchy(input: CoverageHierarchyInput = {}) {
...
export function deriveRomaniaHierarchyFromPlacemark(input: CoverageHierarchyInput & { formattedAddress?: unknown } = {}) {
  ...
  const fallbackCity = fallbackCounty
    ? (
        hierarchy.cityCode
          ...
          : addressParts.map((part) => findRomaniaCity(fallbackCounty.countyCode, part)).find(Boolean)
      ) || null
    : null;
...
export function buildRomaniaCoverageAreaText(input: CoverageHierarchyInput = {}) {
...
export function isRomaniaCoverageHierarchyComplete(input: CoverageHierarchyInput = {}) {
  const resolved = resolveRomaniaCoverageHierarchy(input);

  return Boolean(
    resolved.countryCode === ROMANIA_COUNTRY.countryCode
    && resolved.countyCode
    && resolved.cityCode,
  );
}

export function normalizeRomaniaLocationLabel(value: unknown) {
  return sanitizeText(value);
}
